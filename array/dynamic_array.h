...
template<typename T>
bool DynamicArray<T>::contains(const T &e) {
...
template<typename T>
int DynamicArray<T>::getIndex(const T &e) {
...
template<typename T>
T DynamicArray<T>::get(const size_t &idx) {
    if (idx >= size)
        throw std::runtime_error("访问索引超过当前数组范围！");
    return data[idx];
}
...
template<typename T>
void DynamicArray<T>::set(const size_t &idx, const T &e) {
    if (idx >= size)
        throw std::runtime_error("访问索引超过当前数组范围！");
    data[idx] = e;
}
...
template<typename T>
void DynamicArray<T>::add(const size_t &idx, const T &e) {
    if (idx > size)
...
template<typename T>
T DynamicArray<T>::remove(const size_t &idx) {
    if (idx >= size)
        throw std::runtime_error("访问索引超过当前数组范围！");
    T ret = data[idx];
